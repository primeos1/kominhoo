...
let BUNDLES = JSON.parse(localStorage.getItem('kominhoo_bundles')) || BUNDLES_DEFAULT;

// ── Buying Guides ────────────────────────────────────────────
const GUIDES_DEFAULT = [
  { id:1, icon:"🧴", title:"Acne Solutions", desc:"Clear breakouts fast with targeted ingredients",
    products:[1,4,5,7,13],
    image:"https://images.unsplash.com/photo-1515377905703-c4788e51af15?w=600&h=700&fit=crop&q=80" },
  { id:2, icon:"✨", title:"Glow Boosters", desc:"Achieve glass skin with brightening actives",
    products:[8,12,14,15,6],
    image:"https://images.unsplash.com/photo-1596462502278-27bfdc403348?w=600&h=700&fit=crop&q=80" },
  { id:3, icon:"💧", title:"Hydration Heroes", desc:"Lock in moisture for plump, bouncy skin",
    products:[2,3,16,17,20],
    image:"https://images.unsplash.com/photo-1622618991746-fe6004db3a47?w=600&h=700&fit=crop&q=80" },
  { id:4, icon:"⏰", title:"Anti-Aging Routine", desc:"Fight fine lines with clinically-proven actives",
    products:[8,12,18,15,20],
    image:"https://images.unsplash.com/photo-1556228453-efd6c1ff04f6?w=600&h=700&fit=crop&q=80" },
  { id:5, icon:"🛡️", title:"Sensitive Skin", desc:"Calm redness and strengthen your barrier",
    products:[10,11,2,7,15],
    image:"https://images.unsplash.com/photo-1576426863848-c21f53c60b19?w=600&h=700&fit=crop&q=80" },
  { id:6, icon:"🌟", title:"Pore Perfecting", desc:"Minimize pores and refine skin texture",
    products:[1,5,7,13,19],
    image:"https://images.unsplash.com/photo-1617897903246-719242758050?w=600&h=700&fit=crop&q=80" },
  { id:7, icon:"☀️", title:"Sun Protection", desc:"Find your perfect SPF for Nigerian weather",
    products:[6,14,5],
    image:"https://images.unsplash.com/photo-1598440947619-2c35fc9aa908?w=600&h=700&fit=crop&q=80" },
  { id:8, icon:"🌙", title:"Night Repair", desc:"Maximize skin renewal while you sleep",
    products:[2,9,15,17,18],
    image:"https://images.unsplash.com/photo-1570194065650-d99fb4b38f34?w=600&h=700&fit=crop&q=80" }
];
let GUIDES = JSON.parse(localStorage.getItem('kominhoo_guides')) || GUIDES_DEFAULT;
...
function buyGiftCard() {
  ...
  showToast('🎁', `Gift card for <strong>${recipientName}</strong> added to cart! <a href="${BASE_URL}/checkout" style="color:var(--lime);font-weight:700;text-decoration:underline;margin-left:6px">Checkout →</a>`);
...
function addToCart(productId, fromCheckout) {
  ...
  showToast('🛒', `<strong>${name}</strong> added! <a href="${BASE_URL}/checkout" style="color:var(--lime);font-weight:700;text-decoration:underline;margin-left:6px">Checkout →</a>`);
...
function buildProductCard(p, width = '280px') {
  ...
  <div class="product-card" style="width:${width}">
    <div class="product-img-wrap">
      <a href="${BASE_URL}/shop/product/${p.id}" style="display:block;height:100%">
        <img src="${p.image}" alt="${p.name}" loading="lazy">
      </a>
      <div class="product-badges">${badgeHtml}</div>
      <button class="product-save-btn" onclick="toggleSave(${p.id},this)" title="Save">♡</button>
      <div class="product-actions-overlay">
        <button class="btn btn-white btn-sm" onclick="openQuickView(${p.id})">Quick View</button>
      </div>
    </div>
    <div class="product-info">
      <div class="product-brand">${p.brand}</div>
      <a href="${BASE_URL}/shop/product/${p.id}" class="product-name" style="display:block;color:inherit;text-decoration:none">${p.name}</a>
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:24px;align-items:start">
      ...
      <div>
        ...
        <div style="display:flex;gap:10px">
          ...
          <a href="${BASE_URL}/shop/product/${p.id}" class="btn btn-outline btn-lg" style="flex:1;text-align:center">Full Details →</a>
        </div>
      </div>
    </div>`;
...
function handleSearch(e) {
  ...
  wrap.innerHTML = results.length ? results.slice(0,6).map(p => `
    <a href="${BASE_URL}/shop/product/${p.id}" class="search-result-item" style="display:flex;text-decoration:none;color:inherit">
    ...
    </a>`).join('') : (q.length>=2 ? '<div style="padding:20px;text-align:center;color:var(--text-muted)">No results found</div>' : '');
}
